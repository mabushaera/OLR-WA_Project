...
def rls_KFold(X, y, lambda_, delta, seed):
    """
...
def rls_plot_convergence2(X, y, lambda_, delta, X_test, y_test, model_name):
    """
        Perform Recursive Least Squares with convergence analysis and plot results.
...
def rls_convergence2(X, y, lambda_, delta, model_name, seed):
    kf = KFold(n_splits=5, random_state=seed, shuffle=True)
    rls_list = []
    for train_index, test_index in kf.split(X):
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = y[train_index], y[test_index]
        w, rls_map = rls_plot_convergence2(X, y, lambda_, delta, X_test, y_test, model_name)
