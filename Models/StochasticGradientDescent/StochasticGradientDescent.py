...
def stochastic_gradient_descent_KFold(X, y, epochs, learning_rate, seed):
    """
...
def stochastic_gradient_descent_plot_convergence2(X, y, epochs, learning_rate, X_test, y_test, model_name):
    """
        Perform Stochastic Gradient Descent with convergence analysis and plot results.
...
def stochastic_gradient_descent_convergence2(X, y, epochs, learning_rate, model_name, seed):
    n_splits = 5
    kf = KFold(n_splits=5, random_state=seed, shuffle=True)
    sgd_list = []
    for fold_index, (train_index, test_index) in enumerate(kf.split(X)):
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = y[train_index], y[test_index]
        w, b, sgd_map = stochastic_gradient_descent_plot_convergence2(X_train, y_train, epochs, learning_rate, X_test, y_test, model_name)
