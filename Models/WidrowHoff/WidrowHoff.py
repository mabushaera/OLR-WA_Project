...
def widrow_hoff_plot_convergence2(X, y, learning_rate, X_test, y_test, model_name):
    """
        Perform Widrow-Hoff algorithm with convergence analysis and plot results.
    ...
    return w, widrow_hoff_map

def widrow_hoff_convergence2(X, y, learning_rate, model_name, seed):
    n_splits = 5
    kf = KFold(n_splits=5, random_state=seed, shuffle=True)
    widrow_hoff_list = []
    for fold_index, (train_index, test_index) in enumerate(kf.split(X)):
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = y[train_index], y[test_index]
        w, widrow_hoff_map = widrow_hoff_plot_convergence2(X_train, y_train, learning_rate, X_test,
                                                                      y_test, model_name)
        widrow_hoff_list.append(widrow_hoff_map)

    return widrow_hoff_list
