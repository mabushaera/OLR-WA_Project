...
def online_passive_aggressive_KFold(X, y, C, epsilon, seed):
    """
...
def online_passive_aggressive_plot_convergence2(X, y, C, epsilon, X_test, y_test, model_name):
    """
        Perform Online Passive-Aggressive Regression with convergence analysis and plot results.
...
def online_passive_aggressive_convergence2(X, y, C, epsilon, model_name, seed):
    n_splits = 5
    kf = KFold(n_splits=5, random_state=seed, shuffle=True)
    pa_list = []
    for fold_index, (train_index, test_index) in enumerate(kf.split(X)):
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = y[train_index], y[test_index]
        w, pa_map = online_passive_aggressive_plot_convergence2(X, y, C, epsilon, X_test, y_test, model_name)
