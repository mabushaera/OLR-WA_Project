...
def experiment2Main():
    ...
    # 1. Experiments
    for seed in SEEDS:
        X, y = SyntheticDS.create_dataset(n_samples=n_samples, n_features=n_features, noise=noise, shuffle=True, random_state=seed)
        ...
        olr_wa_acc = OLR_WA.olr_wa_regression_KFold(X, y,
                                              ...
                                              Hyperparameter.olr_wa_increment_size(n_features, user_defined_val=100), seed)
        end_time = time.perf_counter()
        olr_wa_execution_time += (end_time - start_time)
        olr_wa_acc_list_per_seed.append(olr_wa_acc)

        # 4. Online Linear Regression using stochastic gradient descent
        start_time = time.perf_counter()
        ...
        # 5. Online Linear Regression using mini-batch gradient descent
        start_time = time.perf_counter()
        ...
        # 6. Widrow-Hoff
        start_time = time.perf_counter()
        widrow_hoff_acc = WidrowHoff.widrow_hoff_KFold(X, y, Hyperparameter.wf_learning_rate, seed)
        end_time = time.perf_counter()
        widrow_hoff_execution_time += (end_time - start_time)
        widrow_hoff_acc_list_per_seed.append(widrow_hoff_acc)

        # 7. Online Ridge Regression
        start_time = time.perf_counter()
        ...
        # 9. RLS
        start_time = time.perf_counter()
        rls_acc = RLS.rls_KFold(X, y, Hyperparameter.rls_lambda_,
                          Hyperparameter.rls_delta, seed)
        end_time = time.perf_counter()
        ...
        # # 10. OSVR
        # start_time = time.perf_counter()
        # osvr_acc = OnlineSVR.osvr(X, y, Hyperparameter.osvr_C, Hyperparameter.osvr_eps, Hyperparameter.osvr_kernelParam,
        #                           Hyperparameter.osvr_bias, seed)
        # end_time = time.perf_counter()
        # osvr_execution_time = "{:.5f} s".format((end_time - start_time))
        # osvr_list_per_seed.append(osvr_acc)

        # 11. PA
        start_time = time.perf_counter()
        pa_acc = OnlinePassiveAggressive.online_passive_aggressive_KFold(X, y, Hyperparameter.pa_C, Hyperparameter.pa_epsilon,
                                                                   seed)
        end_time = time.perf_counter()
        pa_execution_time += (end_time - start_time)
        pa_list_per_seed.append(pa_acc)

    # 1. Results for Batch Expeiment:
    ...
    # 4. Results for Stochastic Gradient Descent Experiment
    ...
    # 5. Results for Mini-Batch Gradient Descent Experiment
    ...
    # 6. Results for Widrow_Hoff Experiment:
    ...
    # 7. Results for Online Ridge Regression Experiment:
    ...
    # 8. Results for Online Lasso Regression Experiment:
    ...
    # # 10. Results for OSVR Experiment:
    # osvr_acc = np.array(osvr_list_per_seed).mean()
    # print('Online Support Vector Regression, 5 folds, seeds averaging. time:', osvr_execution_time, ', R2:',
    #       "{:.5f}".format(osvr_acc))

    # 12. Results for PA Experiment:
