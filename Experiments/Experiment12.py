...
def experiment12Main():
    """
            ...
            on adversarial scenarios, more specifically on Confidence-Based scenarios where the algorithm suppose to keep
            adapted to the old data as it has higher weights, in this case, the test-data is a portion on the old data.
            It calculates and displays the mean R2 scores and execution times for each model over multiple seed values.

            - Dataset: Synthetic dataset with specified parameters. DS8
            - Models:
    ...
    for seed in SEEDS:
        ...
        X1_train = X1[:train_percent]
        y1_train = y1[:train_percent]
        X_test = X1[train_percent:]
        y_test = y1[train_percent:]

        X, y = Util.combine_two_datasets(X1_train, y1_train, X2, y2)
        ...
        olr_wa_acc = OLR_WA.olr_wa_regression_adversarial(X, y,
                                                          Hyperparameter.olr_wa_w_base_adv2,
                                                          Hyperparameter.olr_wa_w_inc_adv2,
        ...
        # 5. Widrow-Hoff
        start_time = time.perf_counter()
        widrow_hoff_acc = WidrowHoff.widrow_hoff_adversarial(X, y, X_test, y_test, Hyperparameter.wf_learning_rate2)
        end_time = time.perf_counter()
        ...
        # 8. RLS
        start_time = time.perf_counter()
        rls_acc = RLS.rls_adversarial(X, y, X_test, y_test,
